...
class TestDreamExplorerService:
    ...
    async def test_ask_question_basic(self, mock_db, sample_dreams):
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_key'}):
            ...
            with patch.object(service.retrieval_service, 'search_similar_dreams') as mock_search:
                mock_search.return_value = sample_dreams

                # Mock LLM chain
                with patch.object(service.chain, 'arun') as mock_chain:
                    mock_chain.return_value = "Your flying dreams represent freedom and personal growth."
    ...
    async def test_ask_question_with_chat_history(self, mock_db, sample_dreams):
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_key'}):
            ...
            with patch.object(service.retrieval_service, 'search_similar_dreams') as mock_search:
                ...
                with patch.object(service.chain, 'arun') as mock_chain:
                    mock_chain.return_value = "Based on your previous dreams..."
    ...
    async def test_ask_question_custom_top_k(self, mock_db, sample_dreams):
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_key'}):
            ...
            with patch.object(service.retrieval_service, 'search_similar_dreams') as mock_search:
                ...
                with patch.object(service.chain, 'arun') as mock_chain:
                    mock_chain.return_value = "Test response"
    ...
    async def test_find_patterns(self, mock_db, sample_dreams):
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_key'}):
            ...
            with patch.object(service.retrieval_service, 'search_similar_dreams') as mock_search:
                ...
                with patch.object(service.llm, 'apredict') as mock_llm:
                    mock_llm.return_value = "Pattern analysis: You often dream about flying..."
    ...
    async def test_find_patterns_custom_top_k(self, mock_db, sample_dreams):
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_key'}):
            ...
            with patch.object(service.retrieval_service, 'search_similar_dreams') as mock_search:
                ...
                with patch.object(service.llm, 'apredict') as mock_llm:
                    mock_llm.return_value = "Pattern analysis..."
    ...
    async def test_compare_dreams(self, mock_db):
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_key'}):
            ...
            with patch.object(service.llm, 'apredict') as mock_llm:
                mock_llm.return_value = "Comparison: Both dreams involve movement..."
    ...
    async def test_ask_question_no_results(self, mock_db):
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'test_key'}):
            ...
            with patch.object(service.retrieval_service, 'search_similar_dreams') as mock_search:
                ...
                with patch.object(service.chain, 'arun') as mock_chain:
                    mock_chain.return_value = "I couldn't find relevant dreams..."
